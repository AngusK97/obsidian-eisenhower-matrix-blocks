...
截止日期按设备本地日历计算。卡片分层显示完整日期与星期缩写（Mon–Sun）、可选的 24 小时时间、剩余/逾期天数和一行截断备注，各单元完整换行。未完成任务 3 天内（含第 3 天、包括逾期）用红色及闹钟，4–7 天绿色，7 天以后使用正常文字色；已完成日期为次要色、不报警。

日期控件直接打开浏览器/系统原生日历，具体外观随平台变化。时间可选填 00:00–23:59，没设置就不显示，清除日期会一并清除时间。

标签数量不限，回车、逗号或换行确认标签，保存时也包含末尾尚未确认的一项；点击 × 可逐项移除。同名标签由固定哈希映射到跨设备一致的颜色，深浅主题均使用可读配色。标签仅属于任务，不写入 Obsidian 全局标签索引。

旧任务无需迁移。**请先把所有设备更新到 2.7.0 或更高版本，再编辑标签或截止时间**：旧版插件重写矩阵时可能丢弃新增字段。
...
隐藏的 Markdown 注释用于保存自定义象限文案、任务所属象限、排序、创建时间和完成时间。建议通过插入命令创建矩阵，并使用界面操作任务，不要手动编写这些元数据。
