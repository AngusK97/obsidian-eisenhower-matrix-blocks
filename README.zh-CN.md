...
| 操作 | 说明 |
|---|---|
| 添加 | 输入任务名称，可选填截止日期和备注；只填名称也可添加，新任务出现在象限顶部。 |
| 编辑 | 单击进行中或已完成任务，即可编辑名称、截止日期和多行备注；名称中回车保存，备注中 Ctrl/Cmd+Enter 保存。 |
...
截止日期按设备本地日历计算，卡片显示具体日期、剩余或逾期天数及一行截断备注。未完成且距离截止日期不足 3 天（包括已逾期）的任务显示红色闹钟；已完成任务不显示紧急状态。日期气泡提供“今天”“明天”“清除”，自选日期点击“确认日期”或回车后生效。

旧任务无需迁移。**请先把所有设备更新到 2.6.0 或更高版本，再编辑带截止日期或备注的任务**：旧版插件重写矩阵时可能丢弃这些新增字段。
