...
(async () => {
	...
	try {
		for (const [width, narrow, lang] of [[1440, false, "en"], [1440, true, "zh"], [390, false, "zh"], [320, false, "en"]]) {
			...
			assert.equal(await page.locator(".qt-layout-select").inputValue(), "vertical");
			await page.reload();
			assert.equal(await page.locator(".qt-layout-select").inputValue(), "vertical");
			assert.equal(await columns(), 1);
			await page.locator(".qt-layout-select").selectOption("grid");
			await page.reload();
			assert.equal(await page.locator(".qt-layout-select").inputValue(), "grid");
			assert.equal(await columns(), 2);
			await page.locator(".qt-layout-select").selectOption("auto");
			await page.reload();
